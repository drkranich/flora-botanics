 ...
 *
 * Todos os relatórios do sistema usam esta função.
 * Cada tipo de PDF pode ter estilos próprios via categoryStyles.
 */

// ─── Categorias de PDF suportadas ────────────────────────────────────────────
...
export const PDF_CATEGORIES = {
  ...
  relatorio_estoque:  "Relatório de Estoque",
  relatorio_vendas:   "Relatório de Vendas",
  relatorio_crm:      "Relatório CRM / Clientes",
  relatorio_auditoria: "Relatório de Auditoria",
  relatorio_remessa:  "Relatório de Remessas",
  relatorio_pdv:      "Relatório PDV / Caixa",
  contabil:           "Documento Contábil",
  fiscal:             "Documento Fiscal",
  exportacao:         "Documento de Exportação",
  etiqueta:           "Etiqueta / Romaneio",
  assinatura:         "Documento para Assinatura",
  interno:            "Documento Interno",
} as const;

export type PdfCategory = keyof typeof PDF_CATEGORIES;
...
export interface PdfConfig {
  ...
  accentColor?: string;
  headerBorderColor?: string;
  /** Cor do texto corrido (padrão: #1a1a1a) */
  textColor?: string;
  fontFamily?: string;
  watermarkOpacity?: number;
  watermarkSize?: number;
  ...
  /** Mapa categoria → override de estilo */
  categoryStyles?: Partial<Record<PdfCategory, CategoryStyle>>;
}
...
export function buildFloraKraftPDF(options: PdfBuildOptions): string {
  ...
  const catStyle: CategoryStyle = (category && config.categoryStyles?.[category]) ?? {};
  const eff = {
    bgColor:             catStyle.bgColor             ?? config.bgColor             ?? "#f2e8d9",
    accentColor:         catStyle.accentColor         ?? config.accentColor         ?? "#2a4a2c",
    headerBorderColor:   catStyle.headerBorderColor   ?? config.headerBorderColor   ?? "#5a3e2b",
    textColor:           catStyle.textColor           ?? config.textColor           ?? "#1a1a1a",
    fontFamily:          catStyle.fontFamily          ?? config.fontFamily          ?? "Georgia, 'Times New Roman', serif",
    watermarkOpacity:    catStyle.watermarkOpacity    ?? config.watermarkOpacity    ?? 6,
    watermarkSize:       catStyle.watermarkSize       ?? config.watermarkSize       ?? 260,
  };
  ...
  const accentRgb  = hexToRgb(eff.accentColor);
  const borderRgb  = hexToRgb(eff.headerBorderColor);
...
<head>
  ...
  <style>
    ...
    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    @page { margin: 0; background: ${eff.bgColor}; }
    html, body {
      background: ${eff.bgColor} !important;
      color: ${eff.textColor};
      font-family: ${eff.fontFamily};
      font-size: 13px;
      line-height: 1.65;
      min-height: 100%;
    }
    ...
    .pdf-header {
      ...
      border-bottom: 2px solid ${eff.headerBorderColor};
      padding-bottom: 16px;
      margin-bottom: 28px;
      gap: 24px;
    }
    ...
    pre {
      background: rgba(${borderRgb},0.07);
      padding: 6px 8px;
      border-radius: 4px;
      font-size: 10px;
      ...
      color: ${eff.textColor};
      border-left: 3px solid rgba(${borderRgb},0.3);
    }
    .notes-box {
      background: ${accentBg};
      border: 1px solid ${accentBd};
    ...
    .pdf-footer .footer-main { margin-bottom: 4px; }
    .pdf-footer .footer-signer {
      font-size: 10px;
      font-style: italic;
      margin-top: 4px;
      opacity: 0.85;
